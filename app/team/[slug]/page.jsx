...
export function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: { title: d.name, description: d.role, images: d.image ? [d.image] : [] },
  };
}
      ...
      <section className="section">
        <div className="container-px grid gap-10 lg:grid-cols-3">
          <div className="lg:col-span-1">
            ...
            <div className="mt-5 card">
              <ul className="space-y-3 text-sm">
                {doctor.credentials && <li className="flex items-center gap-3"><GraduationCap className="h-4 w-4 text-brand-green-600" /><span className="font-semibold">Credentials:</span> {doctor.credentials}</li>}
                {doctor.gender && <li className="flex items-center gap-3"><UserRound className="h-4 w-4 text-brand-green-600" /><span className="font-semibold">Gender:</span> {doctor.gender}</li>}
                {doctor.languages?.length > 0 && <li className="flex items-center gap-3"><Languages className="h-4 w-4 text-brand-green-600" /><span className="font-semibold">Languages:</span> {doctor.languages.join(', ')}</li>}
              </ul>
          ...
          <div className="lg:col-span-2">
            ...
            <p className="lead mt-4 whitespace-pre-line">{doctor.bio}</p>
